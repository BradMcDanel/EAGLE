...
class Task:
    ...
    def get_model_id(self) -> str:
        ...
        # This will be generated by eval_eagle.py's get_model_id function
        # We just need it for checking if output exists
...
def generate_tasks(config: Dict[str, Any]) -> List[Task]:
    ...
    for dataset in datasets:
        for temp in temperatures:
            ...
            # Eagle uncapped baseline (use first draft_tokens value)
            draft_token = config["draft_tokens"][0]
            tasks.append(Task(
                dataset=dataset,
                temperature=temp,
                mode="eagle_uncapped",
...
def run_task(task: Task, gpu_ids: List[int]) -> Dict[str, Any]:
...
def main():
    ...
    with ProcessPoolExecutor(max_workers=num_workers) as executor:
        ...
        for i, task in enumerate(pending_tasks):
            ...
            task_gpus = gpu_list[start_gpu_idx:start_gpu_idx + gpus_per_run]

            futures.append(executor.submit(run_task, task, task_gpus))
